...
# 加密「团队第三方档案 API key」所用的 Fernet key（可选的独立密钥）。
# 留空时由 SECRET_KEY 确定性派生，因此默认部署无需额外密钥。
#
# 注意：本项**故意留在核心**——`app/utils/secrets.py` 是通用加解密工具，
# 核心 `Team` 的密钥字段依赖它，不是归档导入专属配置（详见 docs §5）。
