...
class ArchiveImportSchema(DefaultSchema):
    ...
    def verify_not_empty(self, data, **kwargs):
        if "gid" in data and not data["gid"].strip():
            raise ValidationError("gid 不能为空", "gid")
        if "token" in data and not data["token"].strip():
            raise ValidationError("token 不能为空", "token")
...
def normalize_archive_api_url(
    ...
    *,
    ...
    if parsed.scheme != "https":
        raise ValueError("归档 API 地址必须使用 HTTPS")
    if not parsed.hostname or parsed.username or parsed.password:
        raise ValueError("归档 API 地址无效")
    if parsed.fragment or parsed.query:
        raise ValueError("归档 API 地址不能包含 query 或 fragment")
    ...
    try:
        ...
        if host in {"localhost", "localhost.localdomain"} or host.endswith(
            ...
            raise ValueError("归档 API 地址不能指向本地或内部主机")
    else:
        if not address.is_global:
            raise ValueError("归档 API 地址不能指向私有或保留 IP")
    ...
    if require_allowlist and not host_allowed:
        raise ValueError("团队归档 API 地址不在站点允许的主机白名单中")
    return value.rstrip("/")
