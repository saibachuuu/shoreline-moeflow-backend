...
class JoinProcessAPITestCase(MoeAPITestCase):
    ...
    def test_team_invite2(self):
        ...
        # user1 加入为普通成员
        user1.join(team1)
        self.assertEqual(user1.get_relation(team1).role.system_code, "member")
        # 邀请user2成为成员
        data = self.post(
            f"/v1/teams/{team1.id}/invitations",
            json={
    ...
    def test_team_invite3(self):
        ...
        self.assertEqual(user2.get_relation(team1).role.system_code, "member")
        # Invitation变成ALLOW
        invitation.reload()
    ...
    def test_change_team_invite1(self):
        ...
        test_role2 = team1.create_role("tr2", 550, [TeamPermission.INVITE_USER])
